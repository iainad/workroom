...
protocol CommandRunning: Sendable {
    func run(_ executable: String, _ args: [String], in directory: String, timeout: TimeInterval) async -> String?
}
...
struct BranchResolver {
    ...
    private func resolveJJ(_ dir: String) async -> RootRef {
        ...
        if let out = await run(dir, "jj", "log", "-r", "@", "--no-graph", "--color", "never",
                               "-T", "bookmarks"),
        ...
        if let out = await run(dir, "jj", "log", "-r", "heads(::@ & bookmarks())", "--no-graph",
